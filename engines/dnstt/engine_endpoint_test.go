...
// TestDNSTTEngineWrapperEndpointIsReal démontre, pour le moteur DNSTT,
...
func TestDNSTTEngineWrapperEndpointIsReal(t *testing.T) {
	// loadDNSTTConfig substitue le port par défaut (53) quand Port<=0 :
	// passer par le wrapper (comme le fait réellement CompositeEngine)
	// exige donc un port explicite et réellement libre.
	cfgPath := filepath.Join(t.TempDir(), "dnstt.json")
	cfg := DNSTTConfig{Domain: "t.example.com", Port: freeUDPPort(t), Backend: "127.0.0.1:0"}
	...
	w := &DNSTTEngineWrapper{configPath: cfgPath}
	...
	ep, ready := w.Endpoint()
	if !ready {
		t.Fatal("Endpoint() pas prêt juste après Start() alors que le socket UDP est ouvert de façon synchrone dans NewDNSTTServer")
	}
	if ep.Network != "udp" {
